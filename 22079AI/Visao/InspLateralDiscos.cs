...
using static _22079AI.Receitas;
...
namespace _22079AI
{
	public class InspLateralDiscos
	{
		...
		public int numberInspThreads = 0;
		...
		private List<HDevProcedureCall> inspScript = new List<HDevProcedureCall>(10);
        private readonly object SendVariablesLock1 = new object();
        private readonly object SendVariablesLock2 = new object();
        ...
        private static readonly Object lockFifo = new Object();
		...
		private void EnviaResultadosPLC()
		{
			...
			while (this.isAlive)
			{
				if ((Forms.MainForm.PlcControl.PlcReadyForNewDIsc))
				{
					if (Measure1 == true)
					{
		...
		private void InitializeHalconScript()
		{
			if (!this.loadedScript)
				try
				{
					...
					this.hEngine.SetEngineAttribute("execute_procedures_jit_compiled", this.jitEnabled ? "true" : "false");
					...
					for (int i = 0; i < 10; i++)
					{
						inspScript.Add(new HDevProcedureCall(new HDevProcedure(this.scriptName)));
						
					}
					  

					while (!this.inspScript[0].IsInitialized() && iLoadCount < 10)
					{
						iLoadCount++;
						Thread.Sleep(100);
					}

					this.loadedScript = this.inspScript[0].IsInitialized();


					//Procall
					//Scripts HDevEngine
					//this.inspScript1 = new HDevProcedureCall(new HDevProcedure(this.scriptName));
		...
		private void DisposeHalconScript()
		{
			if (this.loadedScript)
			{
				if (this.inspScript[0] != null)
				{
					// this.InspScript.GetProcedure().Dispose();
					//this.inspScript.Dispose();
					//this.inspScript1 = null;

					for (int i = 0; i < inspScript.Count; i++)
					{
						inspScript[i].Dispose();
					}
		...
		private bool IniciaCamera()
		{
			...
			try
			{
				HOperatorSet.OpenFramegrabber("GenICamTL", 0, 0, 0, 0, 0, 0, "progressive", -1, "default", -1, "false", "default", "DEV_1AB22C01D76F", 0, -1, out acqHandle);

						  //open_framegrabber('GenICamTL', 0, 0, 0, 0, 0, 0, 'progressive', -1, 'default', -1, 'false', 'default', 'DEV_1AB22C01D76F', 0, -1, AcqHandle)
				...
				try
				{
					...
					if (this.EstadoVisao && !this.sistemaOff) //acquisição contínua da imagem
						try
						{
							...
							this.CamLive.DisplayImage(tempImg, numeroAquisicoes);

							//Debug.WriteLine("Adquiriu a imagem: " + numeroAquisicoes +" "+ DateTime.Now.ToString());

							ListaImagensPorInspecionar.Enqueue(tempImg);
		...
		private void ProcessaImagens()
		{
			...
			DiscCom _DiscId = new DiscCom(),DiscInPLC , DiscInHmi;
			...
			int TimeOutSp = 5000;// tempo maximo para rececao do ID caso contrario limpa a imagem
			int SpTimeOutNewID = 5000;
			bool Measure1 = true;
			//Limpeza para o PLC
			...
			//DiscInPLC = Forms.MainForm.PlcControl.PlcHmiNewDisc;
			//DiscInHmi = Forms.MainForm.PlcControl.HmiPlcFeedbackdisc;

            HDevProcedureCall ScriptEmUso = null;
			int engineThread = 0;

			while (this.isAlive)
			{
				//if (Forms.MainForm.PlcControl.HmiPlcFeedbackdisc.menber.ID != Forms.MainForm.PlcControl.PlcHmiNewDisc.menber.ID)
				if (Forms.MainForm.PlcControl.HmiNewDiscRead)
				{
					if (Measure1 == true)
					{
					...
					if (ListaImagensPorInspecionar.TryDequeue(out AuxImage))
					{
						if (AuxImage != null)
						{
							...
							//NumeroThreads++;
							Debug.WriteLine("Vai inspecionar a imagem: " + Forms.MainForm.PlcControl.PlcHmiNewDisc.menber.ID +" Tempo Captura: " +(DateTime.Now - TimeOutNewID).TotalMilliseconds.ToString() + " ms");
							...
							//ScriptEmUso = inspScript[NumeroThreads];


                            //InspectionThread[NumeroThreads] = new Thread(delegate ()
                            // {
                            //     CallInspection1(AuxImage.Clone(), DiscId, ScriptEmUso);
                            ...
                            ScriptEmUso = inspScript[engineThread];



							Thread myNewThread = new Thread(() => CallInspection(AuxImage, DiscId, ScriptEmUso, engineThread));
                            myNewThread.Start();
				...
				if (((DateTime.Now - TimeOutAq).TotalMilliseconds > TimeOutSp) && PeekElement && ListaImagensPorInspecionar.TryPeek(out AuxImage2))
				{
					if (ListaImagensPorInspecionar.TryDequeue(out AuxImage))
					{
						...
						PeekElement = false;
						string filename = _DiscId.menber.ID + "_Result_" + _DiscId.menber.result + "_" + TimeOutAq.ToString("dd-MM-yyyy") + "_" + DateTime.Now.ToString("HH_mm_ss_ms");

                        StructImagens NewElement = new StructImagens();
						NewElement.Image = AuxImage2;
						NewElement.FilePath = this.diretorioImagens + filename;
		...
		private void CallInspection(HObject ImagemInInspecao, DiscCom DiscId, HDevProcedureCall Script, int EngineThread)

        {
			...
			DateTime startTime = DateTime.Now, AuxDT1;
			double AuxDT2;
			...
			//ReturnInspInfo.TempoInspecao = 0;

			Thread SaveImagesFile = new Thread(GravaImagem);
			Thread WriteDbProdValues = new Thread(EscreveValoresProducaoDb);
			...
			try
			{
				...
				if (true)
				{
                    ...
                    this.CallInspectionScript(ImagemInInspecao, out _ResultadoInspecao, out erroInspecao, out msgInspecao, out ReturnInspInfo.ParamEncascado, out ReturnInspInfo.Regions, Script, EngineThread);
					this.ResultadoInspecao = _ResultadoInspecao;
                    AuxDT2 = (DateTime.Now - AuxDT1).TotalMilliseconds;
					...
					ReturnInspInfo.TempoInspecao = _DiscId.menber.FinalInspecao - _DiscId.menber.InicioInspecao;
					...
					ReturnInspInfo.LastInspectedDisc = _DiscId;
					ListaHmiResultsLastInsp.Enqueue(ReturnInspInfo);


					//Actualiza contadores
					if(_ResultadoInspecao == 1)
					{
						Forms.MainForm.UltimaContagem.totalAprovados++;
						Forms.MainForm.UltimaContagem.parcialAprovados++;
					}
					else if(_ResultadoInspecao == 2)
					{
						Forms.MainForm.UltimaContagem.totalErros++;
						Forms.MainForm.UltimaContagem.parcialErros++;
					}
					else
					{
						Forms.MainForm.UltimaContagem.totalReprovados++;
						Forms.MainForm.UltimaContagem.parcialReprovados++;
					}
					...
					Debug.WriteLine("### Terminou Inspeção com ID: " + _DiscId.menber.ID +" - Lista PLC: " + ListaDiscosInspecionados.Count + " ### Numero Threads: " + EngineThread + " ### Tempo PLC: " + (DateTime.Now - startTime).TotalMilliseconds.ToString("F1") + "ms - Tinsp: " + AuxDT2.ToString("F1") +  " ### Resultado: " + _DiscId.menber.result);
					...
					DB400.INSPECTION_RESULT.MsgErro = msgInspecao;
					...
					if (this.saveImage && this.guardarImagemFimInspecao)
					{
						...
						string filename = this.TempoInicioInspecao.ToString("dd-MM-yyyy") + "_" + this.TempoInicioInspecao.ToString("HH_mm_ss_ms") + " ID_" + _DiscId.menber.ID.ToString() + "_Result_" + _ResultadoInspecao.ToString();

						//string filename = "Imagem " + numImg;
						//numImg++;
						...
						StructImagens NewElement = new StructImagens();
						NewElement.Image = ImagemInInspecao;
						...
						if (!WriteDbProdValues.IsAlive)
							WriteDbProdValues.Start();
					}
                    #endregion
		...
		private void CallInspectionScript(HObject img, out int resultadoInsp, out bool erroInspecao, out string msgErro, out Receitas.ParamEncascado ParametersUsed, out HObject regions, HDevProcedureCall inspScript, int EngineThread)
		{
			...
			try
			{
				...
				if (!modoSimulacao)
				{
                    ...
                    inspScript.SetInputIconicParamObject("InputImage", img);
					inspScript.SetInputCtrlParamTuple("ParamEncascado", SetArrayParamEncascado());
					...
					if (this.timeoutToInspection <= 0)
					{
						...
						this.RunMethodWithTimeout(() => inspScript.Execute(), this.timeoutToInspection);


					//Obtem as outputs do script
					outputImage = inspScript.GetOutputIconicParamObject("OutputImage");
					regions = inspScript.GetOutputIconicParamObject("Region");
					resultadoInsp = inspScript.GetOutputCtrlParamTuple("Resultado").I;
					msgErro = inspScript.GetOutputCtrlParamTuple("MsgErro");
					erroInspecao = inspScript.GetOutputCtrlParamTuple("Erro") == 1;
					ParametersUsed = GetArrayParamEncascado(inspScript.GetOutputCtrlParamTuple("ParamSaida"));
			...
			{
				GC.Collect();
				//Debug.WriteLine("CallInspectionScript - Elapsed time: " + Convert.ToInt32((endTime - startTime).TotalMilliseconds) + " ms - ProcCall: " + EngineThread);
			}
		}
		...
		public void EscreveValoresProducaoDb()
		{
			string strQueryProdTable = string.Empty;
			HmiReadData LastResults = new HmiReadData();

			strQueryProdTable = "INSERT INTO PRODUCAO ([ID_PLC], [ID_HMI], [PULSE_S1], [ENTRADA_S1], [INICIO_INSPECAO], [RESULTADO], [TEMPO_CAPTURA], [TEMPO_INSPECAO], [COMPRIMENTO], [ESPESSURA], [TONALIDADE], [DATA_HORA] ) VALUES (@ID_PLC, @ID_HMI, @PULSE_S1, @ENTRADA_S1, @INICIO_INSPECAO, @RESULTADO, @TEMPO_CAPTURA, @TEMPO_INSPECAO, @COMPRIMENTO, @ESPESSURA, @TONALIDADE, GETDATE())";
			//Debug.WriteLine("Entrou no metodo EscreveValoresProducaoDb()");
			try
			{
				while (ListaHmiResultsLastInsp.TryDequeue(out LastResults))
				{
					//Debug.WriteLine("Vai escrever para a DB no metodo EscreveValoresProducaoDb()");
					using (SqlConnection sqlConn = new SqlConnection(Forms.MainForm.VariaveisAuxiliares.DatabaseConnectionString))
					{
						using (SqlCommand sqlCmd = new SqlCommand(strQueryProdTable, sqlConn))
						{
							sqlCmd.Parameters.Add("@ID_PLC", SqlDbType.BigInt).Value = Convert.ToInt64(LastResults.LastInspectedDisc.menber.ID);
							sqlCmd.Parameters.Add("@ID_HMI", SqlDbType.BigInt).Value = Convert.ToInt64(LastResults.LastInspectedDisc.menber.ID);
							sqlCmd.Parameters.Add("@PULSE_S1", SqlDbType.BigInt).Value = Convert.ToInt64(LastResults.LastInspectedDisc.menber.PulseEncoderS1);
							sqlCmd.Parameters.Add("@ENTRADA_S1", SqlDbType.DateTime).Value = LastResults.LastInspectedDisc.menber.EntradaS1;
							sqlCmd.Parameters.Add("@INICIO_INSPECAO", SqlDbType.DateTime).Value = LastResults.LastInspectedDisc.menber.InicioInspecao;
							sqlCmd.Parameters.Add("@RESULTADO", SqlDbType.TinyInt).Value = Convert.ToInt16(LastResults.LastInspectedDisc.menber.result);
							sqlCmd.Parameters.Add("@TEMPO_CAPTURA", SqlDbType.Int).Value = 0;
							sqlCmd.Parameters.Add("@TEMPO_INSPECAO", SqlDbType.Int).Value = LastResults.TempoInspecao.TotalMilliseconds;
							sqlCmd.Parameters.Add("@COMPRIMENTO", SqlDbType.Real).Value = LastResults.ParamEncascado.COMPRIMENTO;
							sqlCmd.Parameters.Add("@ESPESSURA", SqlDbType.Real).Value = LastResults.ParamEncascado.ESPESSURA;
							sqlCmd.Parameters.Add("@TONALIDADE", SqlDbType.Real).Value = LastResults.ParamEncascado.TONALIDADE_MAX;
